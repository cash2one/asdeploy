...
def get_dirlist_from_ftp(file_path = None, project = None, server_idx = 0):
    if not project:
        return None
    if not file_path:
        file_path = ''
    ...
    if not conn.connect(server_idx):
        return None
    root_dir = FTP_APACHE_PATH + project.war_name
    conn.sftp.chdir(path = root_dir + file_path)
    file_list = conn.sftp.listdir(path = '.')
    conn.disconnect()
    return file_list

def get_file_from_ftp(file_path = None, project = None, server_idx = 0):
...
def upload_file_to_ftp(local_file_path = None, ftp_file_path = None, project = None, server_idx = -1, overwrite = False):
...
def rename_file_on_ftp(parent_folder_path = None, old_file_name = None, new_file_name = None, project = None, server_idx = -1):
...
# 备份文件
def backup_file_on_ftp(parent_folder_path = None, file_name = None, backup_file_name = None, project = None, server_idx = -1):
